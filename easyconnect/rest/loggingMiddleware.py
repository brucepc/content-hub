...
class StandardExceptionMiddleware(object):
    def process_exception(self, request, exception):
        ...
        # catch the stopupload from the custom upload handler, we can then suppress 
        # the 'connect lost' errormessaging and handle appropriately client side
        if isinstance(exception, StopUpload):
            return HttpResponse(json.dumps({ 'cancelled' : True }), content_type="application/json")
